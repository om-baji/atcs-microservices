...
class TrafficSignalAPI:
    """Flask API for traffic signal control"""
    ...
    def __init__(self):
        ...
        # Load trained model
        try:
            self.model.load('traffic_model.joblib')
            logger.info("Model loaded successfully")
        except FileNotFoundError:
            logger.warning("No trained model found. Using default timings.")
    ...
    def setup_routes(self):
        ...
        @self.app.route('/predict/<intersection_id>', methods=['POST'])
        def predict(intersection_id: str):
            try:
                data = request.get_json()

                # Validate input
                required_fields = ['vehicle_counts', 'waiting_times']
                if not all(field in data for field in required_fields):
                    return jsonify({'error': 'Missing required fields'}), 400

                # Create or get intersection controller
                if intersection_id not in self.intersections:
                    self.intersections[intersection_id] = IntersectionController(intersection_id)

                intersection = self.intersections[intersection_id]

                # Update vehicle counts
                intersection.update_vehicle_counts(data['vehicle_counts'])

                # Create traffic data object
                traffic_data = TrafficData(
                    timestamp=datetime.now(),
                    north_count=data['vehicle_counts']['north'],
                    south_count=data['vehicle_counts']['south'],
                    east_count=data['vehicle_counts']['east'],
                    west_count=data['vehicle_counts']['west'],
                ...
                # Get prediction
                optimal_timing = self.model.predict(traffic_data)
    ...
    def _is_peak_hour(self) -> bool:
        ...
        return (7 <= current_hour <= 9) or (16 <= current_hour <= 18)
...
def main():
    """Main function to start the traffic signal control system"""
    api = TrafficSignalAPI()
    logger.info("Starting Traffic Signal Control System")
    api.run()
